...
$infoForm.onsubmit = (e) => {
    ...
    if ($infoForm['newPassword'].value !== '') {
        ...
        if ($infoForm['newPasswordCheck'].value === '') {
            $infoForm['newPasswordCheck'].focusAndSelect().setValid(false).nextElementSibling.innerText = '비밀번호를 한 번 더 입력해 주세요.';
            return;
        }
    }
    ...
    if ($infoForm['newPassword'].value.length >= 0) {
    ...
    xhr.onreadystatechange = () => {
        ...
        switch (response['result']) {
            ...
            case 'failure_password_mismatch' :
                dialog.showSimpleOk('개인정보 수정', `입력하신 현재 비밀번호가 일치하지 않습니다.`, () => $infoForm['currentPassword'].focusAndSelect());
                break;
...
$infoForm['nickname'].addEventListener('focusout', () => {
    ...
    xhr.onreadystatechange = () => {
        ...
        if (response['result'] === true) {
            ...
            $infoForm['nickname'].style.filter = 'hue-rotate(135deg)'; // 초록초록
            $infoForm['nickname'].nextElementSibling.style.filter = 'hue-rotate(135deg)'; // 초록초록
        } else {
            $infoForm['nickname'].setValid(false).nextElementSibling.innerText = '이미 사용 중인 닉네임입니다.';
        }
    };
