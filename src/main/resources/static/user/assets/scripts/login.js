...
$loginForm.onsubmit = (e) => {
    ...
    if ($loginForm['password'].value === '') {
        dialog.showSimpleOk('비밀번호', '비밀번호를 입력해 주세요.', () => $loginForm['password'].focusAndSelect());
        return;
    }
    if (!passwordRegex.test($loginForm['password'].value)) {
        dialog.showSimpleOk('비밀번호', '올바른 비밀번호를 입력해 주세요.', () => $loginForm['password'].focusAndSelect());
        return;
    }
