...
$recoverForm['type'].forEach(($input) => {
    // forEach 돌릴 수 있는 이유: name이 type인 input이 여러개라서 $recoverForm['type']은 (유사)배열임.
    $input.addEventListener('input', () => {
        // 'input' 이벤트는 값 변화(체크 상태 변화 포함)가 일어났을 때... 라는 뜻
        if ($recoverForm['type'].value === 'email') {
            ...
            $recoverForm['passwordRecoverEmail'].focus();
...
$recoverForm.onsubmit = (e) => {
    ...
    if ($recoverForm['type'].value === 'email') {
        ...
        if ($recoverForm['emailRecoverNickname'].value === '') {
            $recoverForm['emailRecoverNickname'].setValid(false).focusAndSelect().nextElementSibling.innerText = '닉네임을 입력해 주세요.';
            return;
        }
        if (!nicknameRegex.test($recoverForm['emailRecoverNickname'].value)) {
            $recoverForm['emailRecoverNickname'].setValid(false).focusAndSelect().nextElementSibling.innerText = '올바른 닉네임을 입력해 주세요.';
            return;
        }
        ...
        xhr.onreadystatechange = () => {
            ...
            switch (response['result']) {
                case 'success':
                    dialog.showSimpleOk('이메일 찾기', `요청한 닉네임 "${$recoverForm['emailRecoverNickname'].value}"에 대한 회원의 이메일은 "${response['email']}"입니다.`, () => $recoverForm.reset());
                    break;
                default:
        ...
        if (!emailRegex.test($recoverForm['passwordRecoverEmail'].value)) {
            $recoverForm['passwordRecoverEmail'].setValid(false).focusAndSelect().nextElementSibling.innerText = '올바른 이메일을 입력해 주세요.';
            return;
        }
        ...
        if (!passwordRegex.test($recoverForm['passwordRecoverPassword'].value)) {
            $recoverForm['passwordRecoverPassword'].setValid(false).focusAndSelect().nextElementSibling.innerText = '올바른 비밀번호를 입력해 주세요.';
            return;
        }
        if ($recoverForm['passwordRecoverPasswordCheck'].value === '') {
            $recoverForm['passwordRecoverPasswordCheck'].setValid(false).focusAndSelect().nextElementSibling.innerText = '비밀번호를 한 번 더 입력해 주세요.';
            return;
        }
        ...
        xhr.onreadystatechange = () => {
            ...
            if (xhr.status < 200 || xhr.status >= 300) {
                dialog.showSimpleOk('비밀번호 재설정', `[${xhr.status}] 요청을 처리하는 도중 오류가 발생하였습니다. 잠시 후 다시 시도해 주세요.`);
                return;
            }
            const response = JSON.parse(xhr.responseText);
            switch (response['result']) {
